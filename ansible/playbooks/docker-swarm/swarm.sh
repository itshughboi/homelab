...
admin=10.10.10.8
manager1=10.10.10.31
manager2=10.10.10.32
manager3=10.10.10.33
worker1=10.10.10.36
worker2=10.10.10.37

# Set the workers' hostnames (if using cloud-init in Proxmox it's the name of the VM)
workerHostname1=swarm-worker-1
workerHostname2=swarm-worker-2
...
certName=id_rsa #default cert in proxmox
...
gluster volume start staging-gfs # << Edit by me. This name cfan be cahgned if i want.
...
ssh -tt $user@$manager1 -i ~/.ssh/$certName sudo su <<EOF
mkdir /mnt/Portainer # bind mount on this which is replicated against all nodes. This makes this HA immediately
